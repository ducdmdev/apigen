...
function generateQueryHook(op: IROperation): string {
...
function generateMutationHook(op: IROperation): string {
  const hookName = `use${capitalize(op.operationId)}`
  const responseType = responseTypeName(op)
...
function generateHooks(ir: IR): string {
  const parts: string[] = []
  ...
  for (const op of queryOps) {
    parts.push(generateQueryHook(op))
    parts.push('')
  }

  for (const op of mutationOps) {
    parts.push(generateMutationHook(op))
    parts.push('')
  }
